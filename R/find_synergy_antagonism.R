...
loss <- function(params, psi, deltas) {
  b0 <- params[1]
  b1 <- params[2]
  sum = 0
  for (i in 1:length(psi)) {
    sum = sum + ((psi[[i]] - msm(deltas[[i]], b0, b1)))**2
  }
  return(sum)
}

find_synergy_antagonism <- function(data, deltas, deltas_arr, a_names, w_names, outcome, outcome_type, mu_learner, top_n = 3, seed) {
  ...
  for (var in a_names) {
    ...
    if (!deltas_arr){
      ...
      shifted_data_var <- shifted_data[[var]]
      psi <- list()
      for (i in deltas[[var]]) {
        shifted_data[[var]] <- shifted_data[[var]] + i
        predictions <- sl_fit$predict(sl3::make_sl3_Task(data = shifted_data, covariates = c(a_names, w_names), outcome = outcome))
        effect <- mean(predictions - data[[outcome]])
        psi <- append(psi, effect)
        shifted_data[[var]] <- shifted_data_var
      }
      opt_result <- optim(par = c(0, 0), fn = loss, psi = psi, deltas = deltas[[var]])
      # print(opt_result$par)
      b1 = opt_result$par[2]
      deltas[[var]] = b1
      deltas_new <- append(deltas_new, (deltas[[var]]))
      shifted_data[[var]] <- shifted_data_var + deltas[[var]]
      predictions <- sl_fit$predict(sl3::make_sl3_Task(data = shifted_data, covariates = c(a_names, w_names), outcome = outcome))
      effect <- mean(predictions - data[[outcome]])
      individual_effects_df <- rbind(individual_effects_df, data.frame(Variable = var, Effect = effect))
    }
